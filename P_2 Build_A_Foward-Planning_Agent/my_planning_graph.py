...
class ActionLayer(BaseActionLayer):
    ...
    def _inconsistent_effects(self, actionA, actionB):
        ...
        for a1 in actionA.effects:
            if ~a1 in actionB.effects:  
                return True
        return False
    ...
    def _interference(self, actionA, actionB):
        ...
        # TODO: implement this function
        for a1 in actionA.effects:
          if ~a1 in actionB.preconditions:
              return True
        for a2 in actionB.effects:
          if ~a2 in actionA.preconditions:
              return True
        return False
        raise NotImplementedError
    ...
    def _competing_needs(self, actionA, actionB):
        ...
        """
        #TODO: implement this function
        for a1 in actionA.preconditions:
          for a2 in actionB.preconditions:
            if self.parent_layer.is_mutex(a1,a2):
              return True
        return False    
        raise NotImplementedError
    ...
    def _inconsistent_support(self, literalA, literalB):
        ...
        for l1 in self.parents[literalA]:
            for l2 in self.parents[literalB]:
                if not self.parent_layer.is_mutex(l1,l2):
                    return False
        return True
        raise NotImplementedError
...
class PlanningGraph:
    ...
    def h_levelsum(self):
        ...
        levelsum = 0
        seen = set()
        while not self._is_leveled and len(seen) != len(self.goal):
            current_s_level = self.literal_layers[-1]
            for literal in self.goal:
                if literal in current_s_level and literal not in seen:
                    seen.add(literal)
                    levelsum += len(self.action_layers)
            self._extend()
        return levelsum
    ...
    def h_setlevel(self):
        ...
        goal = self.goal
        comb = combinations(goal, 2)
        while self._is_leveled == False:
            if goal<= self.literal_layers[-1]:
                comb = [p for p in comb if self.literal_layers[-1].is_mutex(*p)]
            if not comb: break
            self._extend()
        return len(self.action_layers)    
